...
class Ng_word(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if not message.content == text:
            await message.delete()
            await message.channel.send(embed=discord.Embed(description=text, color=0xff0000).set_author(icon_url=message.author.avatar.url, name=message.author.display_name).set_footer(text='NG WORD', icon_url='https://illustcut.com/box/hanko/001/ng1.png'))
            return
